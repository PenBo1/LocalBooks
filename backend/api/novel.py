...
@router.get("/detail/{novel_id}", response_model=Novel)
...
@router.get("/chapters/{novel_id}", response_model=List[Chapter])
...
async def get_chapter_content(chapter_id: int = Path(..., description="章节ID")):
    """获取章节内容"""
    try:
        # 从数据库获取章节
        ...
        if not chapter['content'] or not chapter['is_downloaded']:
            # 获取小说信息
            novel = await crud.get_novel(chapter['novel_id'])
            if not novel:
                raise HTTPException(status_code=404, detail=f"小说不存在: {chapter['novel_id']}")
